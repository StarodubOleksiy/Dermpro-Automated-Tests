...
public class CheckoutTest extends BaseTest {
    ...
    @Feature("DermPRO Guest checkout page")
    @Test(groups = {"guest"}, description = "Verify If purchases are not allowed for Guests")
    public void test03VerifyIfPurchaseNotAllowedForGuest() {
        if (!isFeatureEnabled(testData.getStore().getFeatureStates(), "SO-47")) {
            UI.SignIn().verifyLoginPopup();
        } else {
            System.out.println("For current shop is allowed Guest Checkout");
        }
    }
